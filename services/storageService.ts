...
class StorageService {
  ...
  async init(): Promise<void> {
    if (this.isIndexedDBSupported) {
      try {
        ...
        console.warn(
          "IndexedDB initialization failed, falling back to localStorage only:",
          error
        );
  ...
  async saveOriginalImages(images: ImageData[]): Promise<void> {
    ...
    try {
      await Promise.all(
        images.map((img) => this.indexedDB.saveImage(img, "original"))
      );
    } catch (error) {
  ...
  async saveUpdatedImages(images: ImageData[]): Promise<void> {
    ...
    try {
      await Promise.all(
        images.map((img) => this.indexedDB.saveImage(img, "updated"))
      );
    } catch (error) {
